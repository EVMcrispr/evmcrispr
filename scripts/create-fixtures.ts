...
import { buildIpfsTemplate, fetchAppArtifact, IPFS_GATEWAY, parseContentUri } from "../src/helpers";
import { basePath, fetchOrganizationApps, fetchRepo, generateMockDAOFile } from "./helpers/fixtures";
...
const createDAOAppsFixture = async () => {
  ...
  if (!daoAppsResponse.data.organization.apps) {
    throw new Error(`DAO ${DAO_ADDRESS} used to create fixtures doesn't have apps`);
  }
  ...
  iterableContentUris.forEach((contentUri, index) => {
    ...
    fs.writeFileSync(basePath(`artifacts/${parseContentUri(contentUri)}.json`), JSON.stringify(artifact));
  });
  ...
  fs.writeFileSync(basePath("subgraph-data/OrganizationAppsResponse.json"), JSON.stringify(daoAppsResponse));
...
const createRepoFixture = async () => {
  ...
  const repo = reposResponse.data.repos
    ?.filter(({ registry }: { registry: any }) => registry.name === REGISTRY_NAME)
    .pop();
  ...
  fs.writeFileSync(basePath("subgraph-data/RepoResponse.json"), JSON.stringify({ data: { repos: [repo] } }));
