...
const main = async () => {
  ...
  const agent = new Contract(agentApp.address, agentApp.abiInterface, beeSigner0);
...
const processVote = async (txReceipt: TransactionReceipt, evmcrispr: EVMcrispr) => {
  ...
  const [startVoteLog] = txReceipt.logs.filter((log) => log.topics[0] === hashedStartVoteEvent);
  const voteId = startVoteLog.topics[1];

  const votingApp = evmcrispr.appCache.get("voting:0")!;
  const votingBee0 = new Contract(votingApp.address, votingApp.abiInterface, evmcrispr.signer);
...
const checkAccountBalances = async (account: string, signer: Signer) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    console.log("----------------------------------");
    console.log(`${tokenNames[i]}: ${Number(balance) / 1e18} (${balance.toString()})`);
    console.log("----------------------------------");
  }
};
