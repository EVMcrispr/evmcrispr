...
import { GraphQLWrapper, ParseFunction } from "@1hive/connect-thegraph";
...
class MockGraphQLWrapper extends GraphQLWrapper {
  ...
  performQueryWithParser(query: any, args: any, parserFn: ParseFunction): Promise<any> {
    return new Promise((resolve) => {
      ...
      if (argsKeys.find((key) => key === "repoName")) {
         ...
         */
        response = repoResponse.data.repos.find((repo) => repo.name === (args.repoName as string))
          ? repoResponse
          : { data: { repos: [] } };
