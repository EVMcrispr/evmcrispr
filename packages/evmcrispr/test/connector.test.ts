...
import { ParsedApp } from "../src/types";
import { EOA_ADDRESS, DAO, MockConnector } from "./fixtures";
import { expectThrowAsync, isValidArtifact, isValidParsedApp } from "./test-helpers/expects";
...
describe("Connector", () => {
  ...
  describe("repo()", () => {
    it("should find a valid repo", async () => {
      const { codeAddress, contentUri, artifact } = await connector.repo("token-manager", "aragonpm.eth");

      expect(isAddress(codeAddress), "Invalid  repo code address").to.be.true;

      expect(isIpfs.multihash(parseContentUri(contentUri)), "Invalid repo contentUri").to.be.true;
    ...
    it("should fail when fetching a non-existent repo", async () => {
      await expectThrowAsync(() => connector.repo("non-existent-repo", "aragonpm.eth"), { type: ErrorNotFound });
    });
  });
  ...
  describe("organizationApps()", () => {
    ...
    it("should fail when fetching the apps of a non-existent dao", async () => {
      await expectThrowAsync(() => connector.organizationApps(EOA_ADDRESS), { type: ErrorNotFound });
    });
  });
