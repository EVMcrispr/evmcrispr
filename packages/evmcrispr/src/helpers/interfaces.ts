...
const SYSTEM_APPS: Map<string, { name: string; artifact: any }> = new Map([
  ...
  [EVM_SCRIPT_REGISTRY_APP_ID, { name: "evm-script-registry", artifact: artifactEVMScriptRegistry }],
  [KERNEL_APP_ID, { name: "kernel", artifact: artifactKernel }],
]);

export const isSystemApp = (appId: string): boolean => SYSTEM_APPS.has(appId);

export const getSystemApp = (appId: string): { name: string; artifact: any } | undefined => SYSTEM_APPS.get(appId);
