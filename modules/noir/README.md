...
| [@noir:verify](src/helpers/verify.md) | `bool` | Verify an UltraHonk proof off-chain against a verification key, with no deployed verifier needed. The transcript (keccak or poseidon) is auto-detected from the proof JSON; the vkey must come from @noir:vkey with the matching oracle. |
| [@noir:vkey](src/helpers/vkey.md) | `string` | Compile Noir source and return its UltraHonk verification key as 0x-hex bytes, for @noir:verify off-chain checks. Defaults to the keccak (EVM) transcript so it matches proofs from noir:prove; pass oracle:poseidon for bb's native transcript. |
