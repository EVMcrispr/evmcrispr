...
| [@contracts:solidity](src/helpers/solidity.md) ⚗️ | `bytes` | Compile Solidity source (inline text or a http/ipfs URL) and return the creation bytecode, ready for `deploy`. Options: version:<x.y.z>, runs:<n>, optimizer:false, via-ir:true, evm:<version>, contract:<Name>. |
