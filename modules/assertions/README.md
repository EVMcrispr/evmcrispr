...
| [@assertions:at!](src/helpers/at.md) | `number` | Extract a raw 32-byte word from the return data of a call by word index, on-chain. Static layouts only — dynamic types contribute head offsets, not values. |
...
| [@assertions:split!](src/helpers/split.md) | `string` | Split the string return of a call on a delimiter and select one segment, on-chain. Compare the result at the top level of an assertion. |
