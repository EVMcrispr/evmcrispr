...
| [print](src/commands/print.md) | Print values to script output (stdout in the CLI). Arrays render as headerless tables: a flat array as one row, an array of arrays as one row per inner array. |
...
| [@abi.decodeCall](src/helpers/abi.decodeCall.md) | `any` | Decode calldata into `[contract signature [args]]` with human-readable EVML values. |
