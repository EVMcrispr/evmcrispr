...
describeCommand("raw", {
  ...
  cases: [
    ...
    {
      name: "should return raw action with --gas option",
      script: `raw ${target} ${data} --gas 100000`,
      expectedActions: [{ to: target, data, gas: 100000n }],
    },
    {
      name: "should return raw action with --nonce option",
      script: `raw ${target} ${data} --nonce 5`,
      expectedActions: [{ to: target, data, nonce: 5 }],
    },
    {
      name: "should return raw action with --max-fee-per-gas option",
      script: `raw ${target} ${data} --max-fee-per-gas 20e9`,
      expectedActions: [{ to: target, data, maxFeePerGas: 20000000000n }],
    },
    {
      name: "should return raw action with --max-priority-fee-per-gas option",
      script: `raw ${target} ${data} --max-priority-fee-per-gas 2e9`,
      expectedActions: [
        { to: target, data, maxPriorityFeePerGas: 2000000000n },
      ],
    },
    {
      name: "should handle 0x data (no calldata)",
      script: `raw ${target} 0x`,
      expectedActions: [{ to: target }],
    },
    {
      name: "should combine multiple options",
      script: `raw ${target} ${data} 1e18 --from ${from} --gas 100000 --nonce 3`,
      expectedActions: [
        {
          to: target,
          data,
