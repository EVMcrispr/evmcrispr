/**
 * In-memory circom compilation for `@zk:circom.*` and `zk:prove --circom`.
 *
    ...
    if (seg === "..") {
      if (!out.length) {
        throw new ErrorException(
          `@zk:circom: include path escapes its package root: ${path}`,
        );
      }
      out.pop();
    } else {
...
export function resolveInclude(includePath: string, includer: string): string {
  ...
  if (includePath.startsWith("./") || includePath.startsWith("../")) {
    ...
    if (!dir && !includer.includes("/")) {
      throw new ErrorException(
        `@zk:circom: relative include "${includePath}" is not supported in inline source — host the circuit at a URL or include a version-pinned npm path (circomlib@x.y.z/circuits/...)`,
      );
    }
...
async function fetchSource(
  ...
  if (sourceName.startsWith("ipfs://")) {
    if (!ctx.fetchIpfs) {
      throw new ErrorException(
        `@zk:circom: no IPFS resolver available for ${sourceName}`,
      );
    }
    try {
      ...
      throw new ErrorException(
        `@zk:circom: fetching ${sourceName}: ${(err as Error).message ?? err}`,
      );
    }
  }
  if (isUrl(sourceName)) {
    ...
    try {
      ...
      throw new ErrorException(
        `@zk:circom: network error fetching ${sourceName}: ${(err as Error).message}`,
      );
    }
    if (!res.ok) {
      throw new ErrorException(
        `@zk:circom: ${res.status} ${res.statusText} fetching ${sourceName}`,
      );
    }
    return res.text();
  }
  ...
  if (!spec) {
    ...
    throw new ErrorException(
      `@zk:circom: include "${sourceName}" must pin an exact package version, e.g. ${pkg}@${latest ?? "x.y.z"}/… — unpinned npm content cannot be verified`,
    );
  }
  try {
  ...
  while (queue.length) {
    ...
    for (const m of sources[includer].matchAll(INCLUDE_RE)) {
      ...
      if (Object.keys(sources).length >= MAX_SOURCES) {
        throw new ErrorException(
          `@zk:circom: include graph exceeds ${MAX_SOURCES} files`,
        );
      }
      ctx.log?.(`@zk:circom: fetching ${resolved}…`);
...
async function loadCircomWasmFresh(ctx: FetchContext): Promise<Uint8Array> {
  ...
  ctx.log?.(
    "@zk:circom: downloading the circom compiler (~9 MB, first use only)…",
  );
  ...
  if (hex !== CIRCOM_WASM_SHA256) {
    throw new ErrorException(
      `@zk:circom: the downloaded circom compiler does not match its pinned hash (got ${hex})`,
    );
  }
  return wasm;
}
 ...
 * format) — standalone so `@zk:circom.constraints` never loads snarkjs.
 */
export function parseR1csConstraints(r1cs: Uint8Array): number {
  ...
  if (r1cs.length < 12 || view.getUint32(0, true) !== 0x73633172) {
    throw new ErrorException("@zk:circom: malformed r1cs (bad magic)");
  }
  ...
  throw new ErrorException("@zk:circom: malformed r1cs (no header section)");
}
