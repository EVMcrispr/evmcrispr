...
export function parseProofJson(value: unknown, argName = "proof"): ParsedProof {
  ...
  if (!proof || typeof proof !== "object" || !Array.isArray(publicSignals)) {
    throw new ErrorException(
      `<${argName}> must be a JSON object with "proof" and "publicSignals" — bind it with zk:prove`,
    );
  }
  ...
  if (protocol === "groth16") {
    ...
    if (!Array.isArray(pi_a) || !Array.isArray(pi_b) || !Array.isArray(pi_c)) {
      throw new ErrorException(
        `<${argName}> is missing groth16 proof points (pi_a, pi_b, pi_c) — bind it with zk:prove`,
      );
    }
    return {
      protocol,
...
export function parseZkeyProtocol(zkey: Uint8Array): ProofSystem {
  ...
  if (zkey.length < 12 || view.getUint32(0, true) !== 0x79656b7a) {
    throw new ErrorException("zk:prove: malformed zkey (bad magic)");
  }
  ...
  for (let s = 0; s < nSections; s++) {
    ...
    if (type === 1) {
      ...
      throw new ErrorException(
        `zk:prove: unsupported zkey protocol id ${protocolId}`,
      );
    }
    offset += size;
  }
  throw new ErrorException("zk:prove: malformed zkey (no header section)");
}
