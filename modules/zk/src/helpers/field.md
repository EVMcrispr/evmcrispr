...
set $leaf @zk:field(@hash("my secret"))
print "Leaf:" $leaf
```
...
- Every zk helper applies this reduction to its inputs implicitly; use `@zk:field` when you need the reduced value itself (e.g. to compare against a public signal).

## See Also

- [@zk:field.hash](field.hash.md) — keccak256-then-reduce for arbitrary bytes
- [@zk:poseidon](poseidon.md) — hash field elements
