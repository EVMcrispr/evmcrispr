import { defineHelper, ErrorException } from "@evmcrispr/sdk";
import { unzipParam } from "@evmcrispr/sdk/onchain";
import type Lang from "..";
import { genericLane } from "../utils/genericCollections";
import { wordsArg } from "../utils/onchain";

export default defineHelper<Lang>({
  name: "keys",
  description:
    "Entry names of a record (`[a:1 b:2]` or `[name value]` pairs), as an array.",
  compileDescription:
    "The record is the word-pair payload `@zip!` and `@enumerate!` produce, and string names travel as their keccak digests.",
  returnType: "array",
  args: [
    {
      name: "record",
      type: "record",
      description: "Record (entries array) to read the names from",
    },
  ],
  async run(_, { record }) {
    return (record as [string, unknown][]).map(([name]) => String(name));
  },
  compile: async (ctx, node) => {
    if (node.args.length !== 1) {
      throw new ErrorException(
        "@keys! expects a single record argument, e.g. @keys!(@enumerate!($safe::!{getOwners()(address[])}))",
      );
    }
    const generic = await genericLane(ctx, node.args[0], 0, "keys!");
    if (generic) return generic;
    const { payload, elemType, lanes } = await wordsArg(
      ctx,
      node.args[0],
      "keys!",
    );
    return {
      kind: "call",
      param: unzipParam(ctx, payload, 0n),
      cat: "Bytes",
      collection: {
        element: lanes?.[0] ?? { type: elemType },
        transport: "words",
      },
    };
  },
});
