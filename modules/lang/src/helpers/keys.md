...
|------|------|-------------|
| `record` | `record` | Record (entries array) to read the names from (in @keys! a zipped word-pair payload: a nested @zip!/@enumerate! face or a `::` call returning the interleaved pairs) |
