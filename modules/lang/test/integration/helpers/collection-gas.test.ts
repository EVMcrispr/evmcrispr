...
test("measures composed collection word and generic paths", async () => {
 ...
 def @directTwice! "$x: int256 -> int256" ${OPERATIONS_ADDRESS}::{mul(int256,int256)(int256) $x 2}
 def @directAdd! "$a: int256 $b: int256 -> int256" ${OPERATIONS_ADDRESS}::{add(int256,int256)(int256) $a $b}
 def @directCmp! "$a: int256 $b: int256 -> int256" ${OPERATIONS_ADDRESS}::{sub(int256,int256)(int256) $a $b}`;
