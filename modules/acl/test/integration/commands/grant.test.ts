  ...
  cases: [
    {
      ...
      script: `acl:grant MINTER_ROLE on ${TOKEN_DISTRO} to ${SOME_ADDRESS}`,
      expectedActions: [
    ...
    },
    {
      ...
      script: `acl:grant DEFAULT_ADMIN_ROLE on ${TOKEN_DISTRO} to ${SOME_ADDRESS}`,
      expectedActions: [
        encodeAction(TOKEN_DISTRO, "grantRole(bytes32,address)", [
          ZERO_BYTES32,
          SOME_ADDRESS,
    ...
    },
    {
      ...
      script: `acl:grant 42 on ${MANAGER} to ${SOME_ADDRESS}`,
      expectedActions: [
    ...
    },
    {
      ...
      script: `acl:grant 42 on ${MANAGER} to ${SOME_ADDRESS} --delay 86400`,
      expectedActions: [
        encodeAction(MANAGER, "grantRole(uint64,address,uint32)", [
          Num(42n),
          SOME_ADDRESS,
    ...
    },
    {
      ...
      script: `acl:grant ADMIN_ROLE on ${MANAGER} to ${SOME_ADDRESS}`,
      expectedActions: [
        encodeAction(MANAGER, "grantRole(uint64,address,uint32)", [
          Num(0n),
          SOME_ADDRESS,
  ...
  errorCases: [
    {
      ...
      script: `acl:grant MINTER_ROLE on ${TOKEN_DISTRO} to ${SOME_ADDRESS} --delay 3600`,
      error: "--delay only applies to AccessManager",
    },
    {
      name: "should fail for role ids above uint64",
      script: `acl:grant 18446744073709551616 on ${MANAGER} to ${SOME_ADDRESS}`,
      error: "role ids must be integers",
    },
    {
      name: "should fail for malformed hex roles",
      script: `acl:grant 0xabcd on ${TOKEN_DISTRO} to ${SOME_ADDRESS}`,
