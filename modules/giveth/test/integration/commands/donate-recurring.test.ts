...
describe("Giveth > commands > donate-recurring > with wallet", () => {
  ...
  const runBroadcasting = async (script: string) => {
    ...
    const actionCallback = async (action: Action) => {
      ...
      if (receipt.status !== "success") {
        throw new Error(`test transaction reverted: ${tx.to} ${tx.data}`);
      }
      executed.push(tx);
  ...
  it("runs the full lifecycle: create with wrap, adjust with more/less, stop", async () => {
    ...
    expect(recordedRecurringDonations[0]!.txHash).to.match(/^0x[0-9a-f]{64}$/);
  ...
  it("falls back to creating the record when Giveth doesn't know the stream", async () => {
  ...
  it("streams a --tip on top to the Giveth anchor and leaves it alone on stop", async () => {
  ...
  it("resolves an underlying token to its SuperToken, approving and wrapping", async () => {
