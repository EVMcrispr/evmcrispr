...
- Which chain you are on matters: the same target has a different proxy on each chain. The first argument names the chain the target lives on (`eezL1`, `eezL2`, or a chain id); a bare rollup id works for rollups this module has no chain entry for.
