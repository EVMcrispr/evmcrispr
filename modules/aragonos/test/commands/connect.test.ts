...
describe.concurrent(
  'AragonOS > commands > connect <daoNameOrAddress> [...appsPath] <commandsBlock> [--context <contextInfo>]',
  () => {
    let signer: Signer;
    ...
    it('should return the correct actions when defining a complete forwarding path compose of a fee, normal and context forwarder', async () => {
      ...
      expect(forwardingAction).to.deep.equal(expectedForwardingActions);
    });
    ...
    describe.concurrent('when having nested connect commands', () => {
