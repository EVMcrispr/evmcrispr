...
describeHelper("@vault:pendingDeposit", {
  module: "vault",
  preamble: "switch mainnet",
  cases: [
    {
      name: "returns the pending deposit request of the connected account",
      input: `@vault:pendingDeposit(${CENTRIFUGE_JTRSY_VAULT})`,
      validate: (result) => {
    ...
    },
    {
      ...
      input: `@vault:pendingDeposit(${CENTRIFUGE_JTRSY_VAULT} ${SOME_ADDRESS} 0)`,
      validate: (result) => {
        expect(String(result)).to.eq("0");
      },
    },
  ...
  errorCases: [
    {
      ...
      input: `@vault:pendingDeposit(${SOME_ADDRESS})`,
