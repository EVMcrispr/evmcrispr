...
const PROVE = (inputs: string) =>
  `circom:prove $proof --wasm ${WASM_URL} --zkey ${ZKEY_URL} --inputs ${inputs}`;
...
describeCommand("prove", {
  module: "circom",
  preamble: "load circom",
  describeName:
    "Circom > commands > prove <$variable> --wasm/--zkey | --circom [--ptau] --inputs <entries|json>",
  cases: [
    {
...
circom:prove $proof --circom $src --ptau dev --system plonk --inputs [[a 6] [b 7]]`,
      timeout: 60000,
      validate: (_actions, interpreter) => {
        const { proof, publicSignals } = boundProof(interpreter);
    ...
    },
    {
      ...
      script: `circom:prove $proof --wasm ${WASM_URL} --zkey https://zk.test/multiplier2/plonk.zkey --inputs [[a 6] [b 7]]`,
      timeout: 60000,
      validate: (_actions, interpreter) => {
        const { proof, publicSignals } = boundProof(interpreter);
        expect(proof.protocol).to.equal("plonk");
...
sim:fork --using anvil (
  ...
  contracts:deploy $verifier @contracts:solidity(@circom:verifier($src ptau:dev system:plonk) version:0.8.26)
...
sim:fork --using anvil (
  sim:set-balance @me 1000e18
  contracts:deploy $verifier @contracts:solidity(@circom:verifier($src ptau:dev) version:0.8.26)
  circom:prove $proof --circom $src --ptau dev --inputs [[a 3] [b 11]]
  set [$a $b $c $signals] @circom:proof($proof)
  sim:expect @bool(@lang:at($signals 0) == 33)
  sim:expect @bool(@get($verifier "verifyProof(uint256[2],uint256[2][2],uint256[2],uint256[1])(bool)" $a $b $c $signals) == true)
  ...
  errorCases: [
    {
      ...
      script:
        "circom:prove $proof --wasm https://zk.test/multiplier2/circuit.wasm --inputs [[a 1] [b 2]]",
      error: "circom:prove: --zkey is required (or use --circom)",
    },
    {
      name: "should reject --circom combined with --wasm/--zkey",
      script: `circom:prove $proof --circom "src" --wasm ${WASM_URL} --inputs [[a 1] [b 2]]`,
    ...
    },
    {
      ...
      error: "circom:prove: --system requires --circom",
    },
    {
      name: "should reject an unknown --system value",
      script: `${MULTIPLIER_HEREDOC}
circom:prove $proof --circom $src --system stark --inputs [[a 1] [b 2]]`,
      error: 'unknown proof system "stark"',
    },
    {
      name: "should reject --ptau without --circom",
      script: `${PROVE("[[a 1] [b 2]]")} --ptau dev`,
      error: "circom:prove: --ptau requires --circom",
    },
    {
      name: "should reject an invalid --ptau value",
      script: `${MULTIPLIER_HEREDOC}
circom:prove $proof --circom $src --ptau turbo --inputs [[a 1] [b 2]]`,
      error: 'circom:prove: --ptau must be "dev" or a http(s)/ipfs URL',
    },
    {
      name: "should surface circom diagnostics from --circom",
      script: `circom:prove $proof --circom "template Broken() {" --inputs [[a 1] [b 2]]`,
      error: "@circom:circom: compilation failed",
    },
    {
      name: "should fail on invalid inputs",
      script: `${PROVE("'nope'")}`,
      error: "circom:prove: --inputs must be an entries array",
    },
    {
      name: "should fail when inputs JSON is not an object",
      script: `${PROVE("'[1, 2]'")}`,
      error: "circom:prove: --inputs JSON must be an object",
    },
    {
      name: "should fail on a non-URL artifact",
      script: `circom:prove $proof --wasm ./circuit.wasm --zkey ${ZKEY_URL} --inputs [[a 1] [b 2]]`,
      error: "<wasm> must be an http(s):// or ipfs:// URL",
    },
    {
      name: "should fail when an artifact is missing",
      script: `circom:prove $proof --wasm ${WASM_URL} --zkey https://zk.test/missing/final.zkey --inputs [[a 1] [b 2]]`,
      error: "404",
    },
    {
      name: "should surface snarkjs errors on wrong signal names",
      script: PROVE("[[a 3] [wrong 11]]"),
      error: "circom:prove: proving failed",
    },
  ],
});
