...
- Inputs are reduced into the field first ([@circom:field](field.md) semantics), so hex values and negative numbers are accepted.
- Arity is part of the permutation: `@circom:poseidon(1)` and `@circom:poseidon(1 0)` are different hashes.

## See Also

- [@circom:field](field.md) — the input reduction applied to every argument
- [@circom:tree.root](tree.root.md) — Poseidon Merkle trees over field elements
