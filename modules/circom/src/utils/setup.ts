/**
 * In-place trusted setups for `@circom:*` and `circom:prove --circom`.
 *
  ...
  if (args.ptau !== undefined) {
    ...
    if (!ptau) {
      throw new ErrorException(
        `@circom:circom: invalid ptau "${String(args.ptau)}" — supported: ptau:dev, ptau:<url>`,
      );
    }
    options.ptau = ptau;
  }
...
export function parseSystemValue(value: string): ProofSystem {
  ...
  throw new ErrorException(
    `@circom:circom: unknown proof system "${value}" — supported: ${PROOF_SYSTEMS.join(", ")}`,
  );
}
...
async function devPtauFresh(
  ...
  ctx.log?.(`circom setup: generating a local powers-of-tau (2^${power})…`);
...
export async function getPtau(
  ...
  if (spec.kind === "dev") {
    ...
    if (power > DEV_PTAU_MAX_POWER) {
      throw new ErrorException(
        `@circom:circom: circuit too large for ptau:dev (${constraints} constraints needs a 2^${power} ptau, local cap is 2^${DEV_PTAU_MAX_POWER}) — pass ptau:<url> or omit the option to auto-download one`,
      );
    }
  ...
  if (power > AUTO_PTAU_MAX_POWER) {
    throw new ErrorException(
      `@circom:circom: circuit too large for an in-place setup (${constraints} constraints needs a 2^${power} ptau) — build the zkey offline and use --wasm/--zkey`,
    );
  }
  ctx.log?.(
    `circom setup: downloading powers-of-tau 2^${power} (${hezPtauUrl(power)}, can be tens of MB)…`,
  );
...
export interface SetupResult {
  ...
  /** Verification key JSON (zKey.exportVerificationKey), for @circom:verify. */
  vkeyJson: string;
}
 ...
 * `@circom:*` and `circom:prove --circom` resolve through here.
 */
...
async function setupFresh(
  ...
  if (options.system === "groth16") {
    ctx.log?.(
      ":warning: circom setup: DEV-ONLY groth16 setup — anyone can reproduce it and forge proofs; never guard real value with this verifier (plonk setups need no ceremony: system:plonk)",
    );
    ctx.log?.("circom setup: running the groth16 setup…");
    ...
    ctx.log?.(
      `circom setup: running the deterministic ${options.system} setup (security reduces to the powers-of-tau used${options.ptau.kind === "dev" ? " — a LOCAL dev ptau here, so this setup is still dev-only" : ""})…`,
    );
    const setup = snarkjs[options.system].setup;
    if (!setup) {
      throw new ErrorException(
        `@circom:circom: snarkjs has no ${options.system} setup`,
      );
    }
    try {
      await setup(
      ...
      throw new ErrorException(
        `@circom:circom: ${options.system} setup failed — ${(err as Error).message ?? err}. ${options.system} domains can exceed the r1cs constraint count; try a larger ptau via ptau:<url>`,
      );
    }
  }
  ...
  try {
    ...
    throw new ErrorException(
      `@circom:circom: the ${options.system} setup produced a malformed zkey — usually an undersized powers-of-tau; try a larger one via ptau:<url>`,
    );
  }
