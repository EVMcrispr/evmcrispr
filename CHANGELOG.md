...
- The TWAP box appears only once its registration is confirmed, right after the box of the transaction or proposal carrying it (if that fails, only its box says why), is titled *TWAP 4 USDC → WETH*, and reads *Executed 1/4 (avg 2,709.5 USDC/WETH)*, the average price of what executed so far. Its bar has one segment per part: the running segment grows with its time, faint until it executes and solid after; executed segments link to their order on CoW Explorer, expired ones are hatched. A live countdown under the bar reads *Segment closes in 4m 12s* while the running segment has not executed and *Next segment starts in …* once it has (segments open on a fixed schedule).
